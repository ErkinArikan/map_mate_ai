...
public struct MarkdownAttributedStringParser: MarkupVisitor {
    ...
    mutating func parserResults(from document: Document) -> [ParserResult] {
        ...
        func appendCurrentAttrString() {
            if !currentAttrString.string.isEmpty {
                ...
                results.append(.init(attributedString: currentAttrStringToAppend))
            }
        }
        ...
        document.children.forEach { markup in
            ...
            if markup is CodeBlock {
                ...
                results.append(.init(attributedString: attrStringToAppend))
    ...
    mutating public func visitText(_ text: Text) -> NSAttributedString {
        ...
        // Adres tespiti için Regex
        let addressPattern = #"\b\d{1,5}\s\w+(\s\w+)*,\s\w+(\s\w+)*"# // Basit bir adres formatı
        if let regex = try? NSRegularExpression(pattern: addressPattern, options: []) {
            ...
            for match in matches {
                ...
                if let range = Range(matchRange, in: plainText) {
                    let address = plainText[range]
                    // Adresi tıklanabilir yap
                    attributedString.addAttributes(
                        [.foregroundColor: UIColor.systemBlue, .link: "address://\(address)"],
                        range: matchRange
                    )
                }
            }
